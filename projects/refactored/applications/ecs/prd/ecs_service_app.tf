...
module "app_ecs_service" {
  ...
  name                = "${data.terraform_remote_state.dev.outputs.env_tag}-${data.terraform_remote_state.dev.outputs.app_name}-service"
  ...
  container_name      = data.terraform_remote_state.dev.outputs.app_name
  container_port      = var.app_target_group1_port
  deployment_type     = "CODE_DEPLOY"
  ecs_managed_tags    = false
  health_check        = 0
  iam_role            = "arn:aws:iam::${data.terraform_remote_state.dev.outputs.aws_account_id}:role/aws-service-role/ecs.amazonaws.com/AWSServiceRoleForECS"
