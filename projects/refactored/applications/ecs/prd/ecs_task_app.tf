...
module "app_ecs_task" {
  ...
  app_image_repository     = "${data.terraform_remote_state.dev.outputs.aws_account_id}.dkr.ecr.${data.terraform_remote_state.dev.outputs.region}.amazonaws.com/"
  app_image_version        = "latest"
  container_port           = var.app_target_group1_port
  host_port                = 0
  region                   = data.terraform_remote_state.dev.outputs.region
  family                   = data.terraform_remote_state.dev.outputs.app_name
