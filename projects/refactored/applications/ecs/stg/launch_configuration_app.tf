...
module "app_launch_configuration_v1" {
  ...
  name                        = "TF-LC-${data.terraform_remote_state.env.outputs.env_name}-${data.terraform_remote_state.env.outputs.app_name}-v1"
...
module "app_launch_configuration_v2" {
  source                      = "git::https://github.com/H00065796/dissertation//modules/launch_configuration"
  name                        = "TF-LC-${data.terraform_remote_state.env.outputs.env_name}-${data.terraform_remote_state.env.outputs.app_name}-v2"
  associate_public_ip_address = false
  ebs_optimized               = false
  enable_monitoring           = true
  iam_instance_profile        = "arn:aws:iam::${data.terraform_remote_state.env.outputs.aws_account_id}:instance-profile/${module.ec2_ecs_instance_iam_instance_profile.name}"
